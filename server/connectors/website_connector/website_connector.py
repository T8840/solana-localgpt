...
from models.models import (AppConfig, AuthorizationResult, DataConnector,
                           Document, DocumentMetadata, Source)
...
def sync_load_data_from_url(source_id: str, url: str, config: AppConfig, source_type: Source, css_connector: str) -> Document:
    """
    Convert HTML within the element matching css_connector to text, or if css_connector is None, convert the entire page to text.
    """
    tenant_id = config.tenant_id
...
class WebsiteConnector(DataConnector):
    ...
    def sync_load(self, source_id: str) -> List[Document]:
        documents = []
        for url in self.urls:
            parsed_url = urlparse(url)
            if parsed_url.scheme not in ["http", "https"]:
                raise Exception(f"Invalid URL: {url}")
            document = sync_load_data_from_url(source_id, url, self.config, self.source_type, self.css_selector)
            documents.append(document)
            time.sleep(random.uniform(1, 2))
        return documents    