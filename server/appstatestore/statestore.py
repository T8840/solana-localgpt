...
class StateStore:
    ...
    def get_config(self, bearer_token: str) -> Optional[AppConfig]:
        ...
        for row in response.data:
            ...
            if row['bearer'] == bearer_token:
                if row['vectorstore']:
                    return AppConfig(app_id=row['app_id'], tenant_id=row['user_id'], vectorstore_id=row['vectorstore'],bearer=row['bearer'])
                else:
                    return AppConfig(app_id=row['app_id'], tenant_id=row['user_id'],bearer=row['bearer'])
    ...
    def update_vectorstore(self, config: AppConfig, vectorstore: Vectorstore, credentials: Optional[str]):
        ...
        updated_row = False
        for row in response.data:
            if row['user_id'] == config.tenant_id:
                row['vectorstore'] = vectorstore.value
                print("vectorstore",vectorstore.value)
        ...
        existing_credentials = response.data
        print("existing_credentials:",existing_credentials)
        if len(existing_credentials) > 0:
            to_upsert = existing_credentials[0]
    ...
    def load_vectorstore_credentials(self, config: AppConfig, vectorstore: Vectorstore) -> Optional[str]:
        ...
        print("load_vectorstore_credentials_response:",response)
        for row in response.data:
