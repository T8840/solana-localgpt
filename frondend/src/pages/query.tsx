...
import { getFilterTokenAccounts, burnTokens } from '../lib/solanaUtils'; 
import { useConnection, useWallet } from "@solana/wallet-adapter-react"
  ...
  const ProductsTable: FC = function () {
    ...
    const getBotResponse = async (message: string) => {
      ...
      const url = import.meta.env.VITE_SERVER_URL + '/ask-llm';
  ...
  useEffect(() => {
    ...
    fetchBalance();
    // Cleanup function to update the flag when the component is unmounted
    return () => {
        isMounted = false;
    };
}, [publicKey]); // Added publicKey to the dependency list
